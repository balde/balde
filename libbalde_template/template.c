    ...
    const gchar *template_source)
{
    ...
    gchar *escaped_perc_source = g_regex_replace_literal(re_percent, template_source,
        -1, 0, "%%", 0, NULL);
    g_regex_unref(re_percent);

    GSList *blocks = balde_template_parse(escaped_perc_source);
    ...
    for (GSList *tmp = blocks; tmp != NULL; tmp = g_slist_next(tmp)) {
        ...
        switch (node->type) {
            case BALDE_TEMPLATE_CONTENT_BLOCK:
                cblock = node->block;
                g_string_append(parsed, cblock->content);
                break;
